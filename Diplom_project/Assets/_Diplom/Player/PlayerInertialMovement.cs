...
public class PlayerInertialMovement : MonoBehaviour
{
    [SerializeField] private Reference<Vector3> acceleration;
    ...
    {
        body.rotation = Quaternion.Euler(lookDirection.Get());
        if (body != null)
        {
            Vector3 move = moveDirection.Get();
            Vector3 speed = acceleration.Get();
            if (move != Vector3.zero)
            {
                ...
                if (body.velocity.sqrMagnitude < maxVelocitySqr.Get())
                {
                    ...
                    body.AddForce(transform.rotation * new Vector3(move.x * speed.x, 0, move.z * speed.z)
                        * Time.fixedDeltaTime);
                }
            }
            else
